...
class PlayerActivity : BaseActivity<ActivityPlayerBinding>(ActivityPlayerBinding::inflate),
    ...
    private var isArtworkLoadedForCurrentSong = false

    override fun onCreate(savedInstanceState: Bundle?) {
        isIntentProcessed = savedInstanceState?.getBoolean("intent_processed") ?: false
        lastSongId = savedInstanceState?.getString("last_song_id")
        isTransitionStarted = savedInstanceState?.getBoolean("transition_started") ?: false
    ...
    override fun applyInitialTheme() {
        ...
        currentDominantColor = ThemeManager.currentColors ?: Pair(
            getLibraryColor(MultiColorR.attr.mc_track), getLibraryColor(MultiColorR.attr.mc_tick)
        )
    ...
    private fun updatePlayerUIColors(startColor: Int, endColor: Int) {
        ...
        val backgroundColorStateList =
            ColorStateList.valueOf(ContextCompat.getColor(this, R.color.white_66))
        ...
        val trackColor =
            if (currentMode == DATA.MODE_BASIC) getLibraryColor(MultiColorR.attr.mc_track) else startColor
        val tickColor =
            if (currentMode == DATA.MODE_BASIC) getLibraryColor(MultiColorR.attr.mc_tick) else endColor
        ...
        binding.basicColor.strokeWidth =
            if (currentMode == DATA.MODE_BASIC) resources.getDimensionPixelSize(R.dimen.stroke_width_active) else resources.getDimensionPixelSize(
                R.dimen.stroke_width_inactive
            )
        binding.paletteColor.strokeWidth =
            if (currentMode == DATA.MODE_PALETTE) resources.getDimensionPixelSize(R.dimen.stroke_width_active) else resources.getDimensionPixelSize(
                R.dimen.stroke_width_inactive
            )
        binding.whiteColor.strokeWidth =
            if (currentMode == DATA.MODE_WHITE) resources.getDimensionPixelSize(R.dimen.stroke_width_active) else resources.getDimensionPixelSize(
                R.dimen.stroke_width_inactive
            )
    }
    ...
    private fun setupListeners() {
        ...
        binding.moreOptions.setOnClickListener {
            PlayerOptionsBottomSheet(
                ...
                mediaController,
                { musicViewModel.deleteSong(it); nextBtn(animate = false) },
                {}).show(supportFragmentManager, "PlayerOptions")
        }
    ...
    override fun observeViewModel() {
        ...
        nowPlayerViewModel.themeColorMode.collectWithLifecycle(this) {
            currentMode = it; applyCurrentModeColors()
        }
        ...
        nowPlayerViewModel.marqueeEnabled.collectWithLifecycle(this) {
            binding.songName.isSelected = it
        }
        ...
        musicViewModel.event.collectWithLifecycle(this) { event ->
            if (event is MusicEvent.PlaySong) mediaController?.let {
                forcePlaySong(
                    it, event.position, event.keepProgress
                )
            }
        }
    }
    ...
    private fun updateSongUI(song: MusicFiles) {
        ...
        isArtworkLoadedForCurrentSong = false
    ...
    private fun applyArtworkAndPalette(bitmap: Bitmap?) {
        ...
        val checkReady = {
            if (imageLoaded && blurLoaded) {
                isArtworkLoadedForCurrentSong = true
                if (!isTransitionStarted) {
                    isTransitionStarted = true
                    startPostponedEnterTransition()
                }
        ...
        binding.imageBlur.loadBitmap(
            bitmap, blurRadius = 100f, fallback = R.drawable.ic_cover_song_blur, crossfade = false
        ) { blurLoaded = true; checkReady() }
    }
        ...
        mediaController?.let { controller ->
            ...
            if (duration > 0 && binding.waveformSeekBar.sample != null) {
                binding.waveformSeekBar.progress =
                    (controller.currentPosition.toFloat() / duration.toFloat()) * 100
            }
        }
    }
    ...
    private fun playPauseBtn() {
        mediaController?.togglePlayPause(
            binding.playPause, { stopProgressUpdater() }) { startProgressUpdater() }
    }

    private fun prevBtn(animate: Boolean = true, forceIndex: Int = -1) {
        if (animate) {
            animateSkip(toNext = false); return
        }
    ...
    private fun nextBtn(animate: Boolean = true, forceIndex: Int = -1) {
        if (animate) {
            animateSkip(toNext = true); return
        }
    ...
    private fun animateSkip(toNext: Boolean) {
        ...
        lifecycleScope.launch {
            ...
            var dataReady = false
            val onReady = {
                if (animationDone && dataReady) performSkipAndSlideIn(
                    toNext, inX, targetIndex, nextSong
                )
            }

            binding.card.animate().translationX(outX).alpha(0f)
                .setDuration(resources.getInteger(R.integer.anim_duration_skip).toLong())
                .withEndAction { animationDone = true; onReady() }.start()

            if (nextSong != null) {
                val request = ImageRequest.Builder(this@PlayerActivity).data(
                    getSongImageModel(
    ...
    private fun performSkipAndSlideIn(
        ...
        if (toNext) nextBtn(animate = false, forceIndex = targetIndex) else prevBtn(
            animate = false, forceIndex = targetIndex
        )
        binding.card.translationX = inX
        binding.card.animate().translationX(0f).alpha(1f)
            .setDuration(resources.getInteger(R.integer.anim_duration_skip).toLong())
            .withEndAction { isAnimating = false; preloadedBitmap = null }.start()
    }
        ...
        progressJob = lifecycleScope.launch {
            while (isActive) {
                mediaController?.let { controller ->
                    ...
                    val duration = controller.duration
                    if (duration > 0) {
                        if (binding.seekBar.max != (duration.toInt() / 1000)) binding.seekBar.max =
                            duration.toInt() / 1000
                        if (binding.durationTotal.text == "0:00" || binding.durationTotal.text == DATA.UNKNOWN) binding.durationTotal.text =
                            duration.milliseconds.formatAsTime()
                        binding.seekBar.progress = (currentPos / 1000).toInt()
                        if (binding.waveformSeekBar.sample != null) {
                            binding.waveformSeekBar.progress =
                                (currentPos.toFloat() / duration.toFloat()) * 100
                        }
    ...
    override fun onStart() {
        ...
        controllerFuture?.addListener({
            mediaController =
                controllerFuture?.get(); mediaController?.addListener(this); onControllerConnected()
            ...
            if (intentPosition != -1 && !isIntentProcessed) {
                ...
                if (currentItem != null && controller.currentMediaItemIndex == intentPosition && currentItem.mediaId == targetSong?.id) {
                    ...
                    if (viewModel.listSongs.isNotEmpty()) forcePlaySong(controller, intentPosition)
                        ...
                        while (viewModel.listSongs.isEmpty()) delay(30.milliseconds); forcePlaySong(
                        controller, intentPosition
                    )
                    }
                }
            } else {
                ...
                } else if (controller.currentMediaItem == null && viewModel.position != -1) setupMediaItems(
                    controller
                )
            }
            if ((controller.duration / 1000).toInt() > 0) binding.seekBar.max =
                (controller.duration / 1000).toInt()
    ...
    private fun forcePlaySong(
        ...
        controller.setMediaItems(
            mediaItems, pos, currentProgress
        ...
        updateSongUI(song)
        lifecycleScope.launch {
            delay(300.milliseconds); song.path?.let { path ->
            song.id?.let { id ->
                loadWaveform(
                    id, path
                )
    ...
    private fun setupMediaItems(controller: MediaController) {
        ...
        controller.setMediaItems(
            mediaItems, viewModel.position, viewModel.lastProgress
        ); controller.prepare()
    }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putString("last_song_id", lastSongId)
        outState.putBoolean("transition_started", isTransitionStarted)
    }

    override fun onDeviceInfoChanged(deviceInfo: DeviceInfo) {
        binding.waveformSeekBar.alpha =
            if (deviceInfo.playbackType == DeviceInfo.PLAYBACK_TYPE_REMOTE) 0.5f else 1f
    }
    ...
    override fun onIsPlayingChanged(isPlaying: Boolean) {
        ...
        mediaController?.let {
            val duration = it.duration; if (duration > 0) {
            binding.seekBar.progress = (it.currentPosition / 1000).toInt()
            if (binding.waveformSeekBar.sample != null) {
                binding.waveformSeekBar.progress =
                    (it.currentPosition.toFloat() / duration.toFloat()) * 100
            }
    ...
    private fun updateRepeatShuffleIcons(repeatMode: Int, isShuffle: Boolean) {
        binding.repeat.setImageResource(
            when {
                isShuffle -> R.drawable.ic_shuffle_on; repeatMode == Player.REPEAT_MODE_ONE -> R.drawable.ic_repeat_one; else -> R.drawable.ic_repeat
            }
        )
    }
